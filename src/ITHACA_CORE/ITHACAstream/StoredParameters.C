    ...
    , PODConfiguration_(std::make_unique<PODConfiguration>())
{
    ...
    snapshotConfiguration_->set_casenameData(
        ITHACAdict->lookupOrDefault<fileName>("casename", "./"));
    PODConfiguration_->set_fieldlist(static_cast<List<word>>
    ...
    ROMExecutionConfig_->setPressureResolutionKind(
        StrLookup<PressureResolutionKind>(
    {
    ...
    .lookup(ITHACAdict->lookupOrDefault<word>("pressureResolutionKind", ""),
            PressureResolutionKind::Undefined));
    solverConfiguration_->set_nBlocks(ITHACAdict->lookupOrDefault<label>("nBlocks",
            1));
    solverConfiguration_->set_centeredOrNot(
        ITHACAdict->lookupOrDefault<bool>("centeredOrNot", 1));
    HyperreductionConfiguration_->set_interpFieldCentered(
        ITHACAdict->lookupOrDefault<bool>("interpFieldCenteredOrNot", 0));
    // nMagicPoints = ITHACAdict->lookupOrDefault<label>("nMagicPoints", 1);
    HyperreductionConfiguration_->setHRMethod(
        ITHACAdict->lookupOrDefault<word>("HyperReduction", "DEIM"));
    HyperreductionConfiguration_->setHRInterpolatedField(
        ITHACAdict->lookupOrDefault<word>("DEIMInterpolatedField",
                                          "fullStressFunction"));
    HyperreductionConfiguration_->setECPAlgo(
        ITHACAdict->lookupOrDefault<word>("ECPAlgo", "Global"));
    SimulationFlags_->set_onLineReconstruct(
        ITHACAdict->lookupOrDefault<bool>("onLineReconstruct", 0));
    SimulationFlags_->set_forcingOrNot(
        ITHACAdict->lookupOrDefault<bool>("forcingOrNot", 0));
    SimulationFlags_->set_symDiff(ITHACAdict->lookupOrDefault<bool>("symDiff", 0));
    ROMExecutionConfig_->setROMTemporalScheme(
        ITHACAdict->lookupOrDefault<word>("ROMTemporalScheme", "euler"));
    // SOTA can be 0 (no SOTA), D (deterministic version), S (stochastic version)
    ROMExecutionConfig_->setUseSOTA(ITHACAdict->lookupOrDefault<word>("useSOTA",
            "None"));
    ...
    if (ROMExecutionConfig_->useSOTA() != "None")
    {
        Info << "===============================================================" <<
             endl;
        Info << "  RedLUM is launched in " << ROMExecutionConfig_->useSOTA() <<
                "-SOTA mode" << endl;
        Info << "  Ithacadict file will be overidden by parameters specified in m_parameters.C"
             << endl;
        Info << "===============================================================" <<
             endl;
    }
    ...
    runTimeData = new Foam::Time(Foam::Time::controlDictName, ".",
                                 get_casenameData());
    Foam::Time* runTime;

    if (Pstream::parRun())
    {
        Foam::fileName casenamePar = get_casenameData() + "processor" + name(
                                         Pstream::myProcNo());
        Foam::Time* runTimePar = new Foam::Time(Foam::Time::controlDictName, ".",
                                                casenamePar);
        runTime = runTimePar;
    }
    else
    ...
    PODConfiguration_->set_weightBC(ITHACAdict->lookupOrDefault<double>("weightBC",
            0.));
    PODConfiguration_->set_patchBC(ITHACAdict->lookupOrDefault<word>("patchBC",
            "inlet"));
    ...
    {
        snapshotConfiguration_->set_initialTime(
            ITHACAdict->lookupOrDefault<scalar>("InitialTime", 0));
        snapshotConfiguration_->set_finalTime(
            ITHACAdict->lookupOrDefault<scalar>("FinalTime", 100000000000000));
        snapshotConfiguration_->set_nSnapshots(readScalar(
                ITHACAdict->lookup("Nsnapshots")));
        snapshotConfiguration_->set_startTime(Time::findClosestTimeIndex(
                runTime->times(), get_initialTime()));
        snapshotConfiguration_->set_nSnapshots(min(get_nSnapshots(),
                Times.size() - get_startTime()));
        snapshotConfiguration_->set_endTime(get_startTime() + get_nSnapshots() - 1);
        snapshotConfiguration_->set_finalTime(std::stof(
                runTime->times()[get_endTime()].name()));
    }
    else
    {
        snapshotConfiguration_->set_initialTime(
        ...
        snapshotConfiguration_->set_finalTime(
            ITHACAdict->lookupOrDefault<scalar>("FinalTime", 100000000000000));
        snapshotConfiguration_->set_endTime(Time::findClosestTimeIndex(runTime->times(),
                get_finalTime()));
        snapshotConfiguration_->set_startTime(Time::findClosestTimeIndex(
                runTime->times(), get_initialTime()));
    ...
    // Read Initial and last time from the POD dictionary
    const entry* existnsnapSimulation =
        ITHACAdict->findEntry("NsnapshotsSimulation");
    const entry* existLTSimulation = ITHACAdict->findEntry("FinalTimeSimulation");
    scalar InitialTimeSimulation(get_finalTime());
    label startTimeSimulation(Time::findClosestTimeIndex(runTime->times(),
            InitialTimeSimulation));

    if ((existnsnapSimulation) && (existLTSimulation))
    {
        Info << "Error you cannot define LatestTimeSimulation and NSnapShotsSimulation together"
             << endl;
        abort();
    }
    else if (existnsnapSimulation)
    {
        snapshotConfiguration_->set_nSnapshotsSimulation(readScalar(
                    ITHACAdict->lookup("NsnapshotsSimulation")));
        snapshotConfiguration_->set_nSnapshotsSimulation(min(get_nSnapshotsSimulation(),
                Times.size() - startTimeSimulation));
        snapshotConfiguration_->set_endTimeSimulation(startTimeSimulation +
                get_nSnapshotsSimulation() - 1);
        snapshotConfiguration_->set_finalTimeSimulation(std::stof(
                    runTime->times()[get_endTimeSimulation()].name()));
    }
    else
    {
        snapshotConfiguration_->set_finalTimeSimulation(
            ITHACAdict->lookupOrDefault<scalar>("FinalTimeSimulation", 100000000000000));
        snapshotConfiguration_->set_endTimeSimulation(Time::findClosestTimeIndex(
                    runTime->times(), get_finalTimeSimulation()));
        snapshotConfiguration_->set_nSnapshotsSimulation(get_endTimeSimulation() -
    ...
    IOdictionary controlDict(
        IOobject(
            ...
            IOobject::MUST_READ,
            IOobject::NO_WRITE));
    snapshotConfiguration_->set_writeInterval(
        controlDict.lookupOrDefault<double>("writeInterval", 1));
    ...
    while (line.find("simulationType") == string::npos
    ...
    else if (simulationType == "LES")
    {
        ...
        {
            Info << "Only turbulence model Smagorinsky is supported with simulation type LES"
                 << endl;
    ...
    if (SimulationFlags_->useDEIM())
    {
        ...
        {
            set_deltaWeight(ITHACAutilities::getMassMatrixFV(
                                fieldTemplates_->get_nut()).array().pow(-2.0 / 3.0));
        }
...
{
    return get_pathHilbertSpace_fromHS(
               PODConfiguration_->hilbertSpacePOD()[fieldName]);
}
