...
void HyperreductionConfiguration::setTracerGradOnMagicNeighborhoods(
    Foam::PtrList<Foam::volVectorField>& defT, Foam::label nModes)
{
...
void HyperreductionConfiguration::setTracerGradOnMagicPoints(
    Foam::PtrList<Foam::volVectorField>& defT, Foam::label nModes)
{
...
void HyperreductionConfiguration::setDeformationTensorOnMagicNeighborhoods(
    Foam::PtrList<Foam::volTensorField>& defT, Foam::label nModes)
{
    m_deformationTensorOnMagicNeighborhoods.resize(nModes);

    for (Foam::label k = 0; k < nModes; k++)
    {
        m_deformationTensorOnMagicNeighborhoods.set(k,
            new Foam::volTensorField(defT[k]));
    }
}

void HyperreductionConfiguration::setDeformationTensorOnMagicPoints(
    Foam::PtrList<Foam::volTensorField>& defT, Foam::label nModes)
{
...
void HyperreductionConfiguration::setMeanVectorDEIM(const Foam::volVectorField&
        mean)
{
    m_meanVectorDEIM = new Foam::volVectorField(mean);
}

void HyperreductionConfiguration::setMeanScalarDEIM(const Foam::volScalarField&
        mean)
{
    m_meanScalarDEIM = new Foam::volScalarField(mean);
}

void HyperreductionConfiguration::setMeanVectorDEIMMagic(
    const Foam::volVectorField& mean)
{
    m_meanVectorDEIMMagic = new Foam::volVectorField(mean);
}

void HyperreductionConfiguration::setMeanScalarDEIMMagic(
...
{
    m_magicDelta = new
    Foam::GeometricField<Foam::scalar, Foam::fvPatchField, Foam::volMesh>(mD);
}

void HyperreductionConfiguration::initializeHyperreduction(
    std::unique_ptr<PODConfiguration>& PODConfig)
{
    ...
    bool isReducedField = ITHACAutilities::containsSubstring(m_HRInterpolatedField,
        "reduced");
    ...
    if (m_HRMethod == "ECP")
    {
        ...
        {
            m_HRSnapshotsField = "projFullStressFunction_" + std::to_string(
                                     PODConfig->nModes()["U"]) + "modes";
        }
        else if (ITHACAutilities::containsSubstring(m_HRInterpolatedField,
                "reducedFullStressFunction"))
        {
            m_HRSnapshotsField = "projReducedFullStressFunction_" + std::to_string(
                                     PODConfig->nModes()["U"]) + "modes";
        }
        else if (m_HRInterpolatedField == "nut")
        {
            m_HRSnapshotsField = "projSmagFromNut_" + std::to_string(
                                     PODConfig->nModes()["U"]) + "modes";
        }
        else if (ITHACAutilities::containsSubstring(m_HRInterpolatedField,
                "reducedNut"))
        {
            m_HRSnapshotsField = "projSmagFromReducedNut_" + std::to_string(
                                     PODConfig->nModes()["U"]) + "modes";
        }
        else
        {
            Foam::Info << "Error: ECP method is coded only for fullStressFunction and nut"
                       << Foam::endl;
            abort();
        }
        ...
        {
            for (int k = 0;
                    k <= (c + 1) * (m_ECPAlgo == "EachMode") * (ITHACAutilities::containsSubstring(
                            m_HRInterpolatedField, "nut")); k++)
            {
                ...
                PODConfig->insert_nModes(fieldNameModec,
                                         PODConfig->nModes()[m_HRInterpolatedField]);
    ...
    m_folderDEIM += m_HRInterpolatedField.substr(0,
        m_HRInterpolatedField.find("_")) + "/" + nbModesUInFolderDEIM;
    m_folderDEIM += std::to_string(m_nMagicPoints) + "magicPoints/" +
                    ECPAlgoInFolderDEIM;
}

void HyperreductionConfiguration::initializeReducedField(
...
void HyperreductionConfiguration::addReducedFieldToPOD(const Foam::word&
        nameToReplace,
        ...
        std::unique_ptr<PODConfiguration>& PODConfig)
{
    ...
    PODConfig->appendField(modifiedFieldName, field_type[find(field_name.begin(),
            field_name.end(), nameToReplace) - field_name.begin()]);
    PODConfig->insert_nModes(modifiedFieldName, PODConfig->nModes()[nameToReplace]);
    PODConfig->insert_hilbertSpacePOD(modifiedFieldName,
                                      PODConfig->hilbertSpacePOD()[nameToReplace]);
    PODConfig->set_varyingEnergy(modifiedFieldName, 0);
    PODConfig->set_resolvedVaryingEnergy(modifiedFieldName, 0);
}

Foam::word HyperreductionConfiguration::createReducedFieldName(
