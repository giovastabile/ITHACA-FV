...
PODTemplateH1<T, G>::PODTemplateH1(Parameters* myParameters,
...
Eigen::MatrixXd PODTemplateH1<T, G>::buildCovMatrix()
{
    ...
    Info << "Total varying energy " << l_hilbertSp << " : " << varyingEnergy <<
         endl;
    Info << endl;
    return covMatrix;
}
...
void PODTemplateH1<T, G>::precomputeGradients()
{
    ...
    {
        // Read the j-th field
        local_file = runTime2.caseName() + pathProcessor + runTime2.times()[l_startTime
            + j].name()
                     +  "/" + gradfield_name;
        exist_precomputed_fields = exist_precomputed_fields
                                   && ITHACAutilities::check_file(local_file);
    ...
    if (!exist_precomputed_fields)
    {
        ...
        for (label j = 0; j < l_nSnapshot + l_nSnapshotSimulation - 1; j++)
        {
            ...
            ITHACAstream::read_snapshot(snapshotj, timeFolders[index].name(),
                                        runTime2.caseName());
            gradSnapshotsj = fvc::grad(snapshotj);
...
void PODTemplateH1<T, G>::changeEigenFolderUnitTest(const word& name)
{
    ...
    this->exist_eigenDecomposition = ITHACAutilities::check_file(
                                         this->folder_eigen + this->name_eigenValues + ".npy")
                                     && ITHACAutilities::check_file(this->folder_eigen + this->name_eigenVector +
                                         ".npy");
}
