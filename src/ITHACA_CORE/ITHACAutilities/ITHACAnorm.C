                                ...
                                const double& weightH1)
{
    ...
    {
        Info << "Error: hilbertSpacePOD " << hilbertSpacePOD << " is not valid." <<
             endl;
...
// Specialization
template Eigen::MatrixXd dot_product_POD(PtrList<volTensorField>& v,
        PtrList<volTensorField>& w,
        const word& hilbertSpacePOD,
        const double& weightBC,
        const word& patchBC,
        const double& weightH1);
template Eigen::MatrixXd dot_product_POD(PtrList<volVectorField>& v,
        PtrList<volVectorField>& w,
        const word& hilbertSpacePOD,
        const double& weightBC,
        const word& patchBC,
        const double& weightH1);
template Eigen::MatrixXd dot_product_POD(PtrList<volScalarField>& v,
...
double dot_product_POD(const volVectorField& v, const volVectorField& w,
                       const word& hilbertSpacePOD, const double& weightBC, const word& patchBC,
                       const double& weightH1)
{
    double integral(0);
                       ...
                       const double& weightH1)
{
    ...
    {
        Info << "Error : dot_product_POD cannot be computed between volTensorFields with the hilbertSpacePOD ="
             << hilbertSpacePOD << " ." << endl;
        abort();
    }
    else
    {
...
{
    return ( dot_product_L2(v, w) + weightH1 * dot_product_L2(fvc::grad(v),
            fvc::grad(w)) );
}
// Specialization
template double dot_product_H1(const volVectorField& v, const volVectorField& w,
                               const double& weightH1);
template double dot_product_H1(const volScalarField& v, const volScalarField& w,
                               const double& weightH1);
...
double dot_product_patch(const Eigen::VectorXd& f1BC_i,
                         const Eigen::VectorXd& f2BC_i, const scalarField& AreaFace, const int& d)
{
    ...
    for (label k = 0; k < f1BC_i.size() / d; k++)
    {
        ...
        for (label i = 0; i < d; i++)
        {
            productBC += f1BC_i(k + i * f1BC_i.size() / d) * f2BC_i(
                             k + i * f1BC_i.size() / d);
        }
...
template double dot_product_boundary(const volTensorField& v,
                                     const volTensorField& w, const word& patchBC);
template double dot_product_boundary(const volVectorField& v,
                                     const volVectorField& w, const word& patchBC);
template double dot_product_boundary(const volScalarField& v,
                                     const volScalarField& w, const word& patchBC);

template<typename T>
// Dot product L2wBC : uses dot_product_L2 at the interior and dot_product_boundary at the boundary with a weight weightBC
double dot_product_L2wBC(const T& v, const T& w, const double& weightBC,
                         const word& patchBC)
{
    double dot_prod;
...
template double dot_product_L2wBC(const volTensorField& v,
                                  const volTensorField& w, const double& weightBC, const word& patchBC);
template double dot_product_L2wBC(const volVectorField& v,
                                  const volVectorField& w, const double& weightBC, const word& patchBC);
template double dot_product_L2wBC(const volScalarField& v,
                                  const volScalarField& w, const double& weightBC, const word& patchBC);
...
template double norm_L2wBC(const volTensorField& v, const double& weightBC,
                           const word& patchBC);
template double norm_L2wBC(const volVectorField& v, const double& weightBC,
                           const word& patchBC);
template double norm_L2wBC(const volScalarField& v, const double& weightBC,
                           const word& patchBC);

template<typename T>
double norm_POD(const T& v, const word& hilbertSpacePOD, const double& weightBC,
                const word& patchBC, const double& weightH1)
{
    return std::sqrt(dot_product_POD(v, v, hilbertSpacePOD, weightBC, patchBC,
                                     weightH1));
}
// Specialization
template double norm_POD(const volVectorField& v, const word& hilbertSpacePOD,
                         const double& weightBC, const word& patchBC, const double& weightH1);
template double norm_POD(const volScalarField& v, const word& hilbertSpacePOD,
                         const double& weightBC, const word& patchBC, const double& weightH1);
...
template double frobNorm(GeometricField<scalar, fvsPatchField, surfaceMesh>&
                         field);
