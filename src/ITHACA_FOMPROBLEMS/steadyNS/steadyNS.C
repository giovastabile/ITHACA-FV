...
// Method to solve the supremizer problem  ask Giovanni: Do we need to change this function to consider F_TAUield?
void steadyNS::solvesupremizer(word type)
{
    ...
    if (supex == 1)    // ask Giovanni: what means supex == 1
    {
        volVectorField U = _U();
        volVectorField Usup
        (
...
// Method to compute the lifting function  ask Giovanni: Do we need to make any changes for tau?
...
void steadyNS::projectPPE(fileName folder, label NU, label NP, label NTAU, label NSUP)
{
    NUmodes = NU;
    NPmodes = NP;
...
//***************** AHMAD  ***********************
    if (NTAUmodes != 0)
    {
        for (label k = 0; k < NTAUmodes; k++)
        {
            XImodes.append(XImodes[k].clone());
        }
    }
//************************************************
                                     ...
                                     label NSUP)
{
    ...
    if (ITHACAutilities::check_folder("./ITHACAoutput/Matrices/"))
    {
        ...
        word F_TAU_str = "F_TAU_" + name(liftfield.size()) + "_" + name(NUmodes) + "_" + name(
                         NTAUmodes) + "_" + name(NSUPmodes);

        if (ITHACAutilities::check_file("./ITHACAoutput/Matrices/" + F_TAU_str))
        {
            ITHACAstream::ReadDenseMatrix(F_TAU_matrix, "./ITHACAoutput/Matrices/", F_TAU_str);
        }
        else
        {
            F_TAU_matrix = F_TAU_term(NUmodes, NTAUmodes, NSUPmodes);
        }
        ...
        word K_TAU_str = "K_TAU_" + name(liftfield.size()) + "_" + name(NUmodes) + "_" + name(
                         NSUPmodes) + "_" + name(NTAUmodes);

        if (ITHACAutilities::check_file("./ITHACAoutput/Matrices/" + K_TAU_str))
        {
            ITHACAstream::ReadDenseMatrix(K_TAU_matrix, "./ITHACAoutput/Matrices/", K_TAU_str);
        }
        else
        {
            K_TAU_matrix = K_TAU_term(NUmodes, NTAUmodes, NSUPmodes);
        }
...
Eigen::Tensor<double, 3> steadyNS::G_TAU_tens(label NUmodes,
        label NTAUmodes,
        label NSUPmodes)
{
    label G_TAUsize = NUmodes + NTAUmodes + NSUPmodes + liftfield.size();
    Eigen::Tensor<double, 3> G_TAU_tensor;
    G_TAU_tensor.resize(G_TAUsize, G_TAUsize, G_TAUsize);

    for (label i = 0; i < G_TAUsize; i++)
    {
        for (label j = 0; j < G_TAUsize; j++)
        {
            for (label k = 0; k < G_TAUsize; k++)
            {
...
*/
 ...
 //               if (fluxMethod == "consistent")
                {
                    G_TAU_tensor(i, j, k) = fvc::domainIntegrate(twoSymm(XImodes[i]&fvc::grad(L_U_SUPmodes[j]))&&XImodes[k]).value();
                }
 /*               else
                {
                    G_TAU_tensor(i, j, k) = fvc::domainIntegrate(fvc::grad(
                                            L_U_SUPmodes[i]().T() && 
                                            linearInterpolate(XImodes[j]) & XImodes[j].mesh().Sf(),
                                            XImodes[k]).value();
                }
*/
            }
    ...
    if (Pstream::parRun())
    {
        reduce(G_TAU_tensor, sumOp<Eigen::Tensor<double, 3>>());
    }

    if (Pstream::master())
    {
        // Export the tensor
        ITHACAstream::SaveDenseTensor(G_TAU_tensor, "./ITHACAoutput/Matrices/",
                                      "G_TAU" + name(liftfield.size()) + "_" + name(
                                     NUmodes) + "_" + name(NTAUmodes) + "_" + name(
                                          NSUPmodes) + "_t");
    }
...
Eigen::MatrixXd steadyNS::F_TAU_term(label NUmodes, label NTAUmodes,
        label NSUPmodes)
{
    label P2size = NTAUmodes;
    label P1size = NUmodes + NSUPmodes + liftfield.size();
    Eigen::MatrixXd F_TAU_matrix(P1size, P2size);
    ...
    for (label i = 0; i < P1size; i++)
    {
        for (label j = 0; j < P2size; j++)
        {
            F_TAU_matrix(i, j) = fvc::domainIntegrate(fvc::grad (
                    L_U_SUPmodes[i]) && XImodes[j]).value();
        }
    }

    if (Pstream::parRun())
    {
        reduce(F_TAU_matrix, sumOp<Eigen::MatrixXd>());
    }

    if (Pstream::master())
    {
        ITHACAstream::SaveDenseMatrix(F_TAU_matrix, "./ITHACAoutput/Matrices/",
                                      "F_TAU_" + name(liftfield.size()) + "_" + name(NUmodes) + "_" + name(
                                          NSUPmodes) + "_" + name(NTAUmodes));
    }
...
Eigen::MatrixXd steadyNS::K_TAU_term(label NUmodes, label NTAUmodes,
        label NSUPmodes)
{
    label P2size = NTAUmodes;
    label P1size = NUmodes + NSUPmodes + liftfield.size();
    Eigen::MatrixXd K_TAU_matrix(P1size, P2size);
    ...
    for (label i = 0; i < P1size; i++)
    {
        for (label j = 0; j < P2size; j++)
        {
            ...
            K_TAU_matrix(i, j) = fvc::domainIntegrate(fvc::grad(
                                    L_U_SUPmodes[i])().T() && XImodes[j]).value();
...
//                    L_U_SUPmodes[i]) * XImodes[j]).value();
    ...
    if (Pstream::parRun())
    {
        reduce(K_TAU_matrix, sumOp<Eigen::MatrixXd>());
    }

    if (Pstream::master())
    {
        ITHACAstream::SaveDenseMatrix(K_TAU_matrix, "./ITHACAoutput/Matrices/",
                                      "K_TAU_" + name(liftfield.size()) + "_" + name(NUmodes) + "_" + name(
                                          NSUPmodes) + "_" + name(NTAUmodes));
    }
