...
(Parameters* myParameters, bool mgPoints_0_Neighborhoods_1,
...
void TurbDiffusionHyperreduction<volF, T, S>::computeDefTensor(
    volGradF tensorMeanU)
{
    ...
    {
        m_parameters->set_deformationTensorOfModesOnMagicPoints(
            defTensorOfModesOnMgPointsORMgNeighborhoods);
    }
    else
    {
        m_parameters->set_deformationTensorOfModesOnMagicNeighborhoods(
            defTensorOfModesOnMgPointsORMgNeighborhoods);
    }
}
...
template<typename volF, typename T, typename S>
void TurbDiffusionHyperreduction<volF, T, S>::precomputeTurbDiffusionFunctions(
    word& fieldToCompute)
{
    ...
    if (HRMethod == "ECP"
            && ITHACAutilities::containsSubstring(fieldToCompute, HRSnapshotsField))
    {
        if (ECPAlgo == "Global")
        {
            string pathCentered = "";
            ...
            snapshotsFolder = "./ITHACAoutput/Hyperreduction/" + HRSnapshotsField.substr(0,
                HRSnapshotsField.find("_"))
                              + "/Global" + pathCentered + "/";
            nTotalSnapshots = f_spatialModesU.size() * (l_nSnapshot);

            if (HRInterpField == "nut"
                    || ITHACAutilities::containsSubstring(HRInterpField, "reducedNut"))
            {
                nTotalSnapshots = (f_spatialModesU.size() * (f_spatialModesU.size() + 1) / 2 +
                                   f_spatialModesU.size()) * l_nSnapshot;
            }
        ...
        {
            snapshotsFolder = "./ITHACAoutput/Hyperreduction/" + HRSnapshotsField.substr(0,
                HRSnapshotsField.find("_")) + "/EachMode/";
        }
    ...
    string local_file = snapshotsFolder + pathProcessor + runTime2.times()[1].name()
                        +  "/" + fieldToCompute;
    ...
    {
        // Read the j-th field
        local_file = snapshotsFolder + pathProcessor +
                     ITHACAutilities::double2ConciseString(startingTime + j * saveTime)
                     + "/" + fieldToCompute;
        exist_precomputed_fields = exist_precomputed_fields
                                   && ITHACAutilities::check_file(local_file);
    }
    ...
    if (!exist_precomputed_fields)
    {
        ...
        if (fieldToCompute == HRInterpField || fieldToCompute == "fullStressFunction"
                || fieldToCompute == "nut")
        {
            ...
            if (fieldToCompute == "fullStressFunction"
                    || ITHACAutilities::containsSubstring(fieldToCompute,
                                  "reducedFullStressFunction")
                    || ITHACAutilities::containsSubstring(fieldToCompute, "reducedNut"))
            {
                ITHACAstream::exportSolution(nonLinearSnapshotsj, "0", snapshotsFolder,
                                             fieldToCompute);
                ITHACAstream::exportSolution(nonLinearSnapshotsj, "0",
                                             m_parameters->get_casenameData(), fieldToCompute);
            }
            ...
            for (label j = offset_1; j < nTotalSnapshots; j++)
            {
                ...
                string idxTimeStr = ITHACAutilities::double2ConciseString(
                                        startingTime + j * saveTime);

                if (!ITHACAutilities::containsSubstring(fieldToCompute, "reduced"))
                {
                    ithacaUnsteadyNSTurb->computeNonLinearSnapshot_at_time(snap_time,
                            nonLinearSnapshotsj) ;
                }
                else
                {
                    ithacaUnsteadyNSTurb->computeNonLinearSnapshot_at_time(snap_time,
                            nonLinearSnapshotsj, f_spatialModesU) ;
                }

                ITHACAstream::exportSolution(nonLinearSnapshotsj, idxTimeStr, snapshotsFolder,
                                             fieldToCompute);
            }
        }
        ...
        {
            if (ITHACAutilities::containsSubstring(HRSnapshotsField,
                                                   "projFullStressFunction")
                    || ITHACAutilities::containsSubstring(HRSnapshotsField,
                            "projReducedFullStressFunction"))
            {
                int nUsedModesU = 0;
                int currentModeU = 0;
                ...
                {
                    nUsedModesU = 1;
                    std::vector<int> intInFieldName = ITHACAutilities::extractIntFromString(
                                                          fieldToCompute);
                    currentModeU = intInFieldName[intInFieldName.size() - 1] - 1;
                }

                Info << "Evaluating " << fieldToCompute << " term" << endl;
                ITHACAstream::exportSolution(snapshotECPcj, "0", snapshotsFolder,
                                             fieldToCompute);
                ITHACAstream::exportSolution(snapshotECPcj, "0",
                                             m_parameters->get_casenameData(), fieldToCompute);
                ...
                for (label c = 0; c < nUsedModesU; c++)
                {
                    ...
                    {
                        word snap_time = runTime2.times()[l_startTime + j].name();
                        string idxStr = ITHACAutilities::double2ConciseString(startingTime +
                            (c * l_nSnapshot + j) * saveTime);
                        ithacaUnsteadyNSTurb->computeNonLinearSnapshot_at_time(snap_time, snapshotECPcj,
                                f_spatialModesU[currentModeU]);
                        ITHACAutilities::subtractFields(snapshotECPcj, meanECPModec);
                        ITHACAstream::exportSolution(snapshotECPcj, idxStr, snapshotsFolder,
                                                     fieldToCompute);
                        ITHACAstream::printProgress(double(c * l_nSnapshot + j) /
                                                    (nTotalSnapshots - 1));
                    }
                }
                             ...
                             "projSmagFromReducedNut"))
            {
                ...
                Info << "Evaluating " << fieldToCompute << " term" << endl;
                ITHACAstream::exportSolution(snapshotECPckj, "0", snapshotsFolder,
                                             fieldToCompute);
                ITHACAstream::exportSolution(snapshotECPckj, "0",
                                             m_parameters->get_casenameData(), fieldToCompute);
                ...
                for (label c = 0; c < f_spatialModesU.size(); c++)
                {
                    for (label k = 0; k <= c + 1; k++)
                    {
                        ...
                        else if (ECPAlgo == "Global" && interpFieldCenteredOrNot)
                        {
                            ...
                            ithacaUnsteadyNSTurb->computeProjSmagFromNut_fromNut_fromModes(meanECPModeck,
                                    meanHRInterpField,
                        ...
                        {
                            word snap_time = runTime2.times()[l_startTime + j].name();
                            string idxStr = ITHACAutilities::double2ConciseString(startingTime + (((c *
                                (c + 1) / 2 + c) * l_nSnapshot
                            ...
                            ITHACAstream::exportSolution(snapshotECPckj, idxStr, snapshotsFolder,
                                                         fieldToCompute);
                            ITHACAstream::printProgress(double(((c * (c + 1) / 2 + c)*l_nSnapshot + k *
                                                                l_nSnapshot)
...
endloops:
        ...
        {
            Info << "Error : Unexpected field for precomputeNonPolynomialFunction : " <<
                 fieldToCompute << endl;
            abort();
        }
    }
            ...
            || fieldToCompute == "nut")
    {
        if (interpFieldCenteredOrNot)
        {
            ...
            if (ITHACAutilities::check_file("./ITHACAoutput/mean/" + pathProcessor + "1/" +
                                            fieldToCompute))
            {
            ...
            {
                ITHACAPOD::PODTemplate<T> ithacaFVPOD_interp_field(m_parameters, fieldToCompute,
                        snapshotsFolder);
...
void TurbDiffusionHyperreduction<volF, T, S>::computeTurbDiffusionHyperreduction()
{
    ...
    if (ITHACAutilities::containsSubstring(HRInterpField, "reduced"))
    {
        ...
        snapshotsFolder = "./ITHACAoutput/Hyperreduction/" + HRInterpField.substr(0,
            HRInterpField.find("_")) + "/";
    }
    ...
    else if (HRMethod == "ECP")
    {
        ...
        HRFields_temp.erase(std::remove_if(HRFields_temp.begin(), HRFields_temp.end(),
                                           [HRSnapshotsField_copy](const word x)
        {
            return !ITHACAutilities::containsSubstring(x, HRSnapshotsField_copy);
        }),
        HRFields_temp.end());
    ...
    for (auto fieldIterator : HRPODFields)
    {
        ...
        ITHACAPOD::PODTemplate<S> ithacaFVPOD_template_field(m_parameters,
                fieldIterator, snapshotsFolder);
        ithacaFVPOD_template_field.set_snapFolderParams(POD_nSnapshot,
        ...
        forAll(f_subsetSpatialModesHR, i)
        {
            f_spatialModesHR.append(tmp<S>(f_subsetSpatialModesHR[i]));
        }
    }
    ...
    if (Pstream::parRun())
    {
        ...
        Info << "Hyperreduction POD done in parallel. Magic points selection must run sequentially. Aborting."
             << endl << endl;
        std::exit(111);
    }

    // Convertion to Eigen and weight of the HR modes
    Eigen::VectorXd weights = ITHACAutilities::getMassMatrixFV(
                                  f_spatialModesHR[0]).array().sqrt();
    Eigen::MatrixXd spatialModesHR = weights.asDiagonal() *
                                     Foam2Eigen::PtrList2Eigen(f_spatialModesHR);
    ...
    Eigen::VectorXd fieldWeightsOnes = Eigen::VectorXd::Ones(
                                           ithacaHyperreduction->n_cells * ithacaHyperreduction->vectorial_dim);
    List<Eigen::VectorXd> quadWeights(HRPODFields.size());

    if (HRMethod == "DEIM")
    {
        ithacaHyperreduction->offlineGappyDEIM(spatialModesHR, fieldWeightsOnes,
                                               folder_HR);
    }
    else if (HRMethod == "ECP")
    {
        ...
        if (weightModesWithEigenval)
        {
            for (unsigned int c = 0; c < HRPODFields.size(); c++)
            {
                ...
                std::string folder_eigen = "./ITHACAoutput/EigenValuesandVector" + pathCentered
                                           + "_" +
                ...
                spatialModesHR.middleCols(nModesSubspace * c,
                                          nModesSubspace).array().rowwise() *=
        ...
        else if (ECPAlgo == "EachMode")
        {
            ...
            for (unsigned int c = 0; c < HRPODFields.size(); c++)
            {
                listspatialModesHR[c] = spatialModesHR.middleCols(nModesSubspace * c,
                    nModesSubspace);
            }

            ithacaHyperreduction->offlineECP(listspatialModesHR, fieldWeightsOnes,
                                             folder_HR);
    ...
    if (HRMethod == "DEIM")
    {
        ...
        double cond = svd.singularValues()(0) / svd.singularValues()(
        ...
        cnpy::save(ithacaHyperreduction->MatrixOnline,
                   folder_K_DEIM + "/MatrixOnline.npy");
        m_parameters->set_K_DEIM(ithacaHyperreduction->MatrixOnline);
    }
    else if (HRMethod == "ECP"
             && ITHACAutilities::containsSubstring(HRInterpField, "fullStressFunction"))
    {
        Eigen::VectorXd vectorWeights = ITHACAutilities::getMassMatrixFV(
                                            f_spatialModesU[0]).array().sqrt();
        Eigen::MatrixXd spatialModesU = vectorWeights.asDiagonal() *
                                        Foam2Eigen::PtrList2Eigen(f_spatialModesU);
        ...
        m_parameters->set_K_DEIM(K_DEIM);
             ...
             && ITHACAutilities::containsSubstring(HRInterpField, "nut"))
    {
    ...
    m_parameters->set_magicDelta(ithacaHyperreduction->interpolateField(
                                     m_parameters->get_delta()));
...
template void
TurbDiffusionHyperreduction<volVectorField, volVectorField, volVectorField>::computeDefTensor(
    volTensorField defTensorMean);
template void
TurbDiffusionHyperreduction<volVectorField, volVectorField, volScalarField>::computeDefTensor(
    volTensorField defTensorMean);
template void
TurbDiffusionHyperreduction<volVectorField, volScalarField, volScalarField>::computeDefTensor(
    volTensorField defTensorMean);

template void
TurbDiffusionHyperreduction<volVectorField, volVectorField, volVectorField>::computeTurbDiffusionHyperreduction();
template void
TurbDiffusionHyperreduction<volVectorField, volVectorField, volScalarField>::computeTurbDiffusionHyperreduction();
template void
TurbDiffusionHyperreduction<volVectorField, volScalarField, volScalarField>::computeTurbDiffusionHyperreduction();

template void
TurbDiffusionHyperreduction<volVectorField, volVectorField, volVectorField>::precomputeTurbDiffusionFunctions(
    word& fieldToCompute);
template void
TurbDiffusionHyperreduction<volVectorField, volVectorField, volScalarField>::precomputeTurbDiffusionFunctions(
    word& fieldToCompute);
template void
TurbDiffusionHyperreduction<volVectorField, volScalarField, volScalarField>::precomputeTurbDiffusionFunctions(
    word& fieldToCompute);

template void
TurbDiffusionHyperreduction<volVectorField, volVectorField, volVectorField>::common_MeanHRInterpField();
template void
TurbDiffusionHyperreduction<volVectorField, volVectorField, volScalarField>::common_MeanHRInterpField();
template void
TurbDiffusionHyperreduction<volVectorField, volScalarField, volScalarField>::common_MeanHRInterpField();
