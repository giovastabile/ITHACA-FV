...
int main(int argc, char *argv[])
{
    ...
    DynamicList<label> ownedSampledCellsDyn;
    forAll(sampleLocalCell, sampleI)
    {
        if (sampleLocalCell[sampleI] >= 0)
        {
            ownedSampledCellsDyn.append(sampleLocalCell[sampleI]);
        }
    }
    labelList ownedSampledCells(ownedSampledCellsDyn);

    // Common utility used by both SampledModes and DEIM.
    ...
    fvMeshSubset& subset = sampledMesh.subset();
    fvMesh& subMesh = sampledMesh.subMesh();

    // sampleSubCell is kept in target order for the manual reference checks.
    labelList sampleSubCell =
        buildSampleSubCellMap(sampleLocalCell, subset);

    const label globalSelected =
        sampledMesh.globalSubMeshSize();
    ...
    // Build the expensive mode/submesh mapping ONCE.
    SampledModes<scalar, fvPatchField, volMesh> scalarSampledModes
    (
        scalarITHACAModes,
        sampledMesh,
        nScalarModes
    ...
    // Optimized online call: no subset.interpolate() here.
    List<Eigen::MatrixXd> scalarProjected =
        scalarITHACAModes.projectSampled
        (
            scalarASub,
            scalarSampledModes,
            "PG"
    ...
    const scalar scalarMethodArDiff = relativeDifference(scalarProjected[0], scalarArSub);
    const scalar scalarMethodBrDiff = relativeDifference(scalarProjected[1].col(0), scalarBrSub);
    ...
    // Build the expensive mode/submesh mapping ONCE.
    SampledModes<vector, fvPatchField, volMesh> vectorSampledModes
    (
        vectorITHACAModes,
        sampledMesh,
        nVectorModes
    ...
    // Optimized online call: no subset.interpolate() here.
    List<Eigen::MatrixXd> vectorProjected =
        vectorITHACAModes.projectSampled
        (
            vectorASub,
            vectorSampledModes,
            "PG"
    ...
    const bool scalarPass =
        ...
        && scalarMethodArDiff < tolerance
        && scalarMethodBrDiff < tolerance;
    ...
    if (Pstream::master())
    {
        Info << nl
             ...
             << "rel diff method Ar: " << scalarMethodArDiff << nl
             << "rel diff method br: " << scalarMethodBrDiff << nl
        ...
        Info << nl
             ...
             << "rel diff method Ar: " << vectorMethodArDiff << nl
             << "rel diff method br: " << vectorMethodBrDiff << nl
