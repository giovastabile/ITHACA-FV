    ...
    magicPoints(m_parameters->get_magicPoints())
{
    ...
    {
        Info << "Error: Hyperreduction InterpolatedField not valid: " <<
             interpolatedField << endl;
...
void tutorial27_offline::decompose()
{
    ...
    podU.getModes(spatialModesU, temporalModesU, temporalModesUSimulation,
                  covMatrixU);
    // get mean of U field computed when getModes was used
    meanU = new volVectorField(podU.get_mean());
    m_parameters->set_meanU(*meanU);
    volVectorField template_HRInterpField(interpolatedField,
                                          m_UnsteadyNSTurb->initSmagFunction());
    ...
    if (ITHACAutilities::containsSubstring(interpolatedField, "fullStressFunction"))
    {
        ...
        if (m_parameters->get_HRMethod() == "DEIM")
        {
            ...
            turbDiffHR_fullStressFunction = new
            TurbDiffusionHyperreduction<volVectorField, volVectorField, volVectorField>
            (m_parameters, mgPoints_0_Neighborhoods_1, template_HRInterpField,
             template_HRInterpField, meanU, spatialModesU);
        ...
        else if (m_parameters->get_HRMethod() == "ECP")
        {
            ...
            turbDiffHR_projFullStressFunction = new
            TurbDiffusionHyperreduction<volVectorField, volVectorField, volScalarField>
            (m_parameters, mgPoints_0_Neighborhoods_1, template_HRInterpField,
             template_HRSnapshotsField, meanU, spatialModesU);
    ...
    else if (ITHACAutilities::containsSubstring(interpolatedField, "nut"))
    {
        ...
        volScalarField template_HRInterpField(interpolatedField,
                                              m_UnsteadyNSTurb->initNutFunction());
        volScalarField template_HRSnapshotsField(
            m_UnsteadyNSTurb->initProjSmagFromNutFunction());
...
void tutorial27_offline::project()
{
    ...
    if (ITHACAutilities::containsSubstring(interpolatedField, "fullStressFunction"))
    {
        word file_projK_DEIM = folder_DEIM + "projected_K_DEIM_" + std::to_string(
                                   nModesU) + ".npy";
        ...
        if (!exist_projK_DEIM)
        {
            ...
            for (int i = 0 ; i < nModesU ; i++)
            {
                ...
                if (m_parameters->get_HRMethod() == "DEIM")
                {
                    ...
                    Eigen::VectorXd weightedSpatialModesU = Foam2Eigen::field2Eigen(
                            weightedSpatialModesU_OF);
                ...
                K_proj(i, d * nMagicPoints) = ITHACAutilities::dot_product_L2(spatialModesU[i],
                    meanSmag);
    ...
    else if (ITHACAutilities::containsSubstring(interpolatedField, "nut"))
    {
        ...
        bool exist_xi_onMagicPts = ITHACAutilities::check_file(folder_DEIM +
            xi_onMagicPts_file);
        ...
        if (!exist_xi_onMagicPts)
        {
            ...
            if (m_parameters->get_HRMethod() == "DEIM")
            {
                K_DEIM.array().colwise() /= Foam2Eigen::field2Eigen(
                                                m_parameters->get_volume()).array().sqrt();
                ...
                for (label i = 0; i < nModesU; i++)
                {
                    for (label q = 0; q < nModesU + 1; q++)
                    {
                        for (label j = 0; j < nMagicPoints; j++)
                        {
                            ...
                            if (q == 0)
                            {
                                xi_onMagicPts(q, j, i) = ITHACAutilities::dot_product_L2(spatialModesU[i],
                                    m_UnsteadyNSTurb->diffusion(Foam2Eigen::Eigen2field(meanNut, K_DEIM_col_j),
                                                                *meanU));
                            }
                            else
                            {
                        ...
                        if (q == 0)
                        {
                            xi_onMagicPts(q, nMagicPoints,
                                          i) = ITHACAutilities::dot_product_L2(spatialModesU[i],
            ...
            {
                PtrList<volTensorField> defTensorModesOnMgPts =
                ...
                for (label i = 0; i < nModesU; i++)
                {
                    for (label q = 0; q < nModesU + 1; q++)
                    {
                        for (label j = 0; j < nMagicPoints; j++)
                        {
                            label row_id = ((pow(max(i, q - 1), 2) +3 * max(i, q - 1)) / 2 + min(i + 1,
                                q)) * (m_parameters->get_ECPAlgo() != "Global");
                            xi_onMagicPts(q, j, i) = -2 * K_DEIM(row_id,
                                                                 j) * (dev(defTensorModesOnMgPts[i + 1][localMagicPoints[j]])
