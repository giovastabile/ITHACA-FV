...
{
    ITHACAstream::read_fields(spatialModesU, "U",
                              "./ITHACAoutput/spatialModes_" + std::to_string(nModesU) + "modes/");
    massMatrixInv = ITHACAutilities::getMassMatrix(
                        spatialModesU).completeOrthogonalDecomposition().pseudoInverse();
    cnpy::load(temporalModesUSimulation,
               "./ITHACAoutput/temporalModesSimulation_" + std::to_string(
                   nModesU) + "modes/U.npy");
    weightAtMg = Eigen::VectorXd(nMagicPoints);

    for (int i = 0; i < nMagicPoints; i++)
    {
        weightAtMg(i) = std::pow(m_parameters->get_submesh().V()[localMagicPoints[i]],
                                 0.5);
    }
    ...
    {
        d = 3;
        meanSmagOnMagicNeighborhoods = new volVectorField(
            m_parameters->get_meanVectorDEIMMagic());

        for (auto& defTensorOfMode :
                m_parameters->get_deformationTensorOfModesOnMagicNeighborhoods())
        {
    ...
    {
        d = 1;
        meanNutOnMagicPoints = new volScalarField(
            m_parameters->get_meanScalarDEIMMagic());

        for (auto& defTensorOfMode :
                m_parameters->get_deformationTensorOfModesOnMagicPoints())
        {
            defTensorOfModesAtMg.push_back(defTensorOfMode);
        }
    }
    ...
    nut0 = new volScalarField("nut", volScalarField(
                                  IOobject("nut0", m_parameters->get_casenameData() + "0",
                                           m_parameters->get_submesh(), IOobject::MUST_READ),
    ...
    ccc = new volScalarField(2 * Ck * (*delta) * (dev(*fullDefField)
        && (*fullDefField)));
    sqrtk = new volScalarField((-(*bbb) + sqrt(sqr(*bbb) + 4 * (*aaa) * (*ccc))) *
                               (*inv2aaa));
...
Eigen::VectorXd tutorial27_online::computeApproxSmagMg(const Eigen::VectorXd&
        reducedCoeffs)
{
    evaluateApproxNut(reducedCoeffs);
    volVectorField stressField = fvc::div(2 * (*nut0) * dev(
            *fullDefField)) - *meanSmagOnMagicNeighborhoods;
...
Eigen::VectorXd tutorial27_online::computeApproxNutMg(const Eigen::VectorXd&
        reducedCoeffs)
{
    evaluateApproxNut(reducedCoeffs);
...
Eigen::VectorXd tutorial27_online::predictSmagROMCoeffs(
    const Eigen::VectorXd& reducedCoeffs)
{
    ...
    else if (ITHACAutilities::containsSubstring(interpolatedField, "nut"))
    {
        ...
        Eigen::Tensor<double, 3 > xi_complete_onMagicPts(
            m_parameters->get_xi_onMagicPts());
...
volVectorField tutorial27_online::computeROMproj_fromCoeffs(
    const Eigen::VectorXd& reducedCoeffs, bool stressUnit)
{
    ...
    PtrList<volVectorField> reconstruction = ITHACAutilities::reconstructFromCoeff(
            spatialModesU, reducedCoeffsMatrix, nModesU);

    if (stressUnit)
    {
        // Dummy scalar to homogenized the reconstruction
        dimensionedScalar time_inv("time_inv", dimensionSet(0, 0, -1, 0, 0),
                                   scalar(1.0));
...
Eigen::VectorXd tutorial27_online::computeROMcoeffs_fromFullDim(
    volVectorField& f_full)
{
    bool consider_volumes = true;
    Eigen::MatrixXd coeffsVelocityPOD = ITHACAutilities::getCoeffs(f_full,
        spatialModesU, nModesU, consider_volumes);
    return coeffsVelocityPOD.col(0);
}

// Compute f
volVectorField tutorial27_online::computeSmagTerm_fromChronos(
    const Eigen::VectorXd& reducedCoeffs)
{
    return m_UnsteadyNSTurb->computeSmagTerm_fromU(*meanU +
            computeROMproj_fromCoeffs(reducedCoeffs));
}


void tutorial27_online::prediction()
{
    word folder_results = "./ITHACAoutput/Online/" + m_parameters->get_HRMethod() +
                          "_" + interpolatedField.substr(7)
                          + "U_" + std::to_string(nMagicPoints) + "MgPts/";
    mkDir(folder_results);
    Eigen::MatrixXd predictedCoeffs(m_parameters->get_nSnapshotsSimulation(),
                                    nModesU);
    ...
    {
        label index_time = t + m_parameters->get_startTime() +
                           m_parameters->get_nSnapshots() - 1;
        // Computing prediction
        predictedCoeffs.row(t) = predictSmagROMCoeffs(temporalModesUSimulation.row(t));
        volVectorField predictedSmagProj = computeROMproj_fromCoeffs(
                                               predictedCoeffs.row(t), true);
        // Computing references from reduced velocity
        volVectorField exactSmag = computeSmagTerm_fromChronos(
                                       temporalModesUSimulation.row(t));
        exactCoeffs.row(t) = computeROMcoeffs_fromFullDim(exactSmag);
        volVectorField exactSmagProj = computeROMproj_fromCoeffs(exactCoeffs.row(t),
            true);
        ...
        ITHACAstream::exportSolution(exactSmagProj, idxTimeStr,
                                     folder_results + "/exactSmagProj", "fullStressFunction");
        ITHACAstream::exportSolution(predictedSmagProj, idxTimeStr,
                                     folder_results + "/predictedSmagProj", "fullStressFunction");
    }
