...
class tutorial25: public laplacianProblem
{
public:
    explicit tutorial25(int argc, char* argv[])
        :
    ...
    /// M field
    autoPtr<volScalarField> _M;

    List<labelList> M_list_ind;
    List<Eigen::MatrixXd> M_list;
    ...
    void compute_nu()
    {
        ...
        M_list[0].resize(T.size(),T.size());
        M_list[1].resize(T.size(),T.size());
        M_list[2].resize(T.size(),T.size());
        M_list[3].resize(T.size(),T.size());
        M_list[4].resize(T.size(),T.size());
        M_list[5].resize(T.size(),T.size());
        M_list[6].resize(T.size(),T.size());
        M_list[7].resize(T.size(),T.size());
        M_list[8].resize(T.size(),T.size());
        M_list_ind[0] = ITHACAutilities::getCellIndicesFromBox(_mesh(), Box1);
        M_list_ind[1] = ITHACAutilities::getCellIndicesFromBox(_mesh(), Box2);
        M_list_ind[2] = ITHACAutilities::getCellIndicesFromBox(_mesh(), Box3);
        M_list_ind[3] = ITHACAutilities::getCellIndicesFromBox(_mesh(), Box4);
        M_list_ind[4] = ITHACAutilities::getCellIndicesFromBox(_mesh(), Box5);
        M_list_ind[5] = ITHACAutilities::getCellIndicesFromBox(_mesh(), Box6);
        M_list_ind[6] = ITHACAutilities::getCellIndicesFromBox(_mesh(), Box7);
        M_list_ind[7] = ITHACAutilities::getCellIndicesFromBox(_mesh(), Box8);
        M_list_ind[8] = ITHACAutilities::getCellIndicesFromBox(_mesh(), Box9);
        for (unsigned int k = 0; k < M_list_ind.size(); k++)
        {
            for (unsigned int i = 0; i < M_list_ind[k].size(); i++)
            {
    ...
    Eigen::MatrixXd compute_M_delta()
    {
        unsigned int M_size = T.size();
        Eigen::MatrixXd M_delta(M_size,M_size);

        if (ITHACAutilities::check_file("M_delta.npy"))
        {
            cnpy::load(M_delta,"M_delta.npy");
        }
        else
        {

            volScalarField phi_i(T);
            volScalarField phi_j(T);

            ITHACAutilities::assignBC(phi_i,0,0.0);
            ITHACAutilities::assignBC(phi_j,0,0.0);
            ITHACAutilities::assignBC(phi_i,1,0.0);
            ITHACAutilities::assignBC(phi_j,1,0.0);
            ITHACAutilities::assignBC(phi_i,2,0.0);
            ITHACAutilities::assignBC(phi_j,2,0.0);
            ITHACAutilities::assignBC(phi_i,3,0.0);
            ITHACAutilities::assignBC(phi_j,3,0.0);
            ...
            for (unsigned int i = 0; i< M_size; i++)
            {
                ...
                {
                    ITHACAutilities::assignIF(phi_i,0.0);
                    ITHACAutilities::assignIF(phi_j,0.0);
                    phi_i[i] = 1.0;
                    phi_j[j] = 1.0;
                    scalar M_value =  fvc::domainIntegrate(fvc::grad(phi_i)&fvc::grad(phi_j)).value()/2;
                    M_delta(i,j) = M_value;
                }
            }
            cnpy::save(M_delta,"M_delta.npy");
        }
        return M_delta;
    }
...
void offline_stage(tutorial25& example, tutorial25& FOM_test);
void online_stage(tutorial25& example, tutorial25& FOM_test);
...
int main(int argc, char* argv[])
{
    ...
    /// Create a new instance of the FOM problem for testing purposes
    tutorial25 FOM_test(argc, argv);
    ITHACAparameters* para = ITHACAparameters::getInstance(FOM_test._mesh(),
                             FOM_test._runTime());
...
void offline_stage(tutorial25& example, tutorial25& FOM_test)
{
    ...
    ITHACAparameters* para = ITHACAparameters::getInstance(example._mesh(),
                             example._runTime());
    Eigen::MatrixXd M_delta = FOM_test.compute_M_delta();
...
/// \until cmath
///
/// \subsection classtuto02 Implementation of the tutorial25 class
///
/// Then we can define the tutorial25 class as a child of the laplacianProblem class
///
/// \skipline tutorial25
/// \until {}
///
...
///
/// Once the tutorial25 class is defined the main function is defined,
/// an example of type tutorial25 is constructed:
///
/// \skipline argv)
///
